...
class Body {
	...
	async formData() {
		...
		const { toFormData } = await Promise.resolve().then(function () {
			return require('./multipart-parser-431aff0b.js');
		});
		...
		request_.on('response', (response_) => {
			...
			if (codings === 'deflate' || codings === 'x-deflate') {
				...
				raw.once('data', (chunk) => {
					...
					body =
						(chunk[0] & 0x0f) === 0x08
							? Stream.pipeline(body, zlib__default['default'].createInflate(), reject)
							: Stream.pipeline(body, zlib__default['default'].createInflateRaw(), reject);
